...
When looking at the defense: a negative EP means the defense gave up less points
than expected. For example, an EP on defense of -0.25 means the defense gave up
less points than expected by 0.25 points. Negative and bigger is better.
...
Full R code found in: `EpaPerPlay.R` In order to easily visualize the EPA per
play for each team in each season, we first need to pull the relevant data from
`nflfastR`:
...
where `nflfastR::loadpbp` loads the entire play by play of all teams in all
...
We use the number of wins in a season as our IV on the X axis and our binary
made playoffs as a binary "yes"=1 / "no"=0.
